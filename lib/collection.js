  ...
  FS.readdirSync(dir).forEach(function(itemName) {
    ...
    for (attachment in self.options.attachments) {
      ...
      if (FS.existsSync(attachFile)) {
        if (attachOpts.strategy === 'link') {
          self.items[itemName][attachment] = self.options.baseURL + '/' + itemName + '/' + attachment;
        } else {
...
Collection.prototype.get = function(id) {
  var self = this;
  if (!id) {
    var items = JSON.parse(JSON.stringify(_.values(self.items)));
    if (self.options.sort) items = items.sort(self.options.sort);
    return items;
  } else {
...
Collection.prototype.getAttachment = function(id, attachment, callback) {
  var self = this;
  var ext = self.options.attachments[attachment].extension || '';
  if (ext) ext = '.' + ext;
  var filename = Path.join(self.directory, id, attachment + ext);
...
Collection.prototype.reload = function(itemName, callback) {
  var self = this;
  if (!callback) {
    callback = itemName;
    return this.reloadAll(callback);
  }
...
Collection.prototype.reloadAll = function(callback) {
  var self = this;
  self.items = {};
