...
Gitback.prototype.initialize = function(callback) {
  ...
  self.repo.clone(self.directory, function(err) {
    ...
    var collections = files.filter(function(name) {
      return FS.statSync(Path.join(self.directory, name)).isDirectory()
    });
...
Gitback.prototype.addRoutesForCollection = function(name, col) {
  ...
  var checkAccess = function(method, item, user) {
    if (!col.access[method]) return false;
    if (col.access[method] === 'all') return true;
    return col.access[method](item, user);
  }
  ...
  if (col.access.get) {
    self.router.get('/' + name + '/:id', function(req, res) {
      ...
      try {
        ...
        if (!checkAccess('get', item, req.user)) res.status(401).json({error: 'Not authorized'});
      } catch (e) {
        return res.status(500).json({error: e.message});
      }
      if (checkAccess('get', item, req.user)) {
        res.json(item);
      } else {
    ...
    self.router.get('/' + name, function(req, res) {
      ...
      try {
        items = items.map(function(item) {
          ...
          return checkAccess('get', item, req.user);
        })
      } catch (e) {
        return res.status(500).json({error: e.message});
      }
    ...
    if (col.access[method]) {
      self.router[method]('/' + name, function(req, res) {
        ...
        try {
          ...
          if (checkAccess(method, item, req.user)) {
