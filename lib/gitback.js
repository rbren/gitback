...
Gitback.prototype.initialize = function(callback) {
  ...
  var initFromDir = function(err) {
    ...
    var apiFile = Path.join(self.directory, 'api.js');
...
Gitback.prototype.addRoutesForCollection = function(name, col) {
  ...
  col.baseURL = (self.options.baseURL || '') + '/' + name;
  ...
  if (col.access.get) {
    self.router.get('/' + name + '/:id', signIn, function(req, res) {
      ...
      try {
        item = applyMiddleware('get', item, req.params.id);
    ...
    for (var attachment in col.attachments) {
      var attachOpts = col.attachments[attachment];
      if (attachOpts.strategy === 'link') {
        var path = '/' + name + '/:id/' + attachment + '(.' + attachOpts.extension + ')?';
        self.router.get(path, function(req, res) {
          var item = self.collections[name].get(req.params.id);
          if (!item) return res.status(404).json({error: 'Item ' + req.params.id + ' not found'});
          try {
            if (!checkAccess('get', item, req.user, req.strategy)) return res.status(401).json({error: 'Not authorized'})
          ...
          var data = self.collections[name].getAttachment(req.params.id, attachment, function(err, data) {
            if (err) return res.status(500).json({error: e});
            if (!data) return res.status(404).json({error: "Attachment " + attachment + " not found for item " + req.params.id});
            res.send(data);
          });
        })
      }
...
Gitback.prototype.sync = function(callback) {
  ...
  Async.parallel(Object.keys(self.collections).map(function(c) {
    return function(acb) {
      self.collections[c].reload(function(err) {
